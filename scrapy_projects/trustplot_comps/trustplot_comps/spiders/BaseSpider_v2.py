    ...
    def parse(self, response):
        ...
        def extract_text(link, JavaScript=False):
            ...
            if 'JavaScript' in text:
                extract_text(link, JavaScript=True)
        ...
        if response.status == 200:
            ...
            for url in response.xpath('//a/@href').re(r'.+?contact.+?'):
                if url.__contains__('http'):
                    contact_link = validate_link(url, current_url)
                else:
                    if url.startswith('/'):
                        contact_link = current_url+url[1:]
                    else:
            ...
            print(contact_link)
            try:
                frame['phones'] = find_phones(contact_link)
            except Exception:
                frame['phones'] = find_phones(current_url)
            try:
                frame['address'] = find_address(contact_link)
            except Exception:
                frame['address'] = find_address(current_url)
            try:
                frame['email'] = find_email(contact_link)
            except Exception:
                frame['email'] = find_email(current_url)
            try:
                frame['facebook'] = find_links(contact_link)['facebook']
            except Exception:
                frame['facebook'] = find_links(current_url)['facebook']
            try:
                frame['instagram'] = find_links(contact_link)['instagram']
            except Exception:
                frame['instagram'] = find_links(current_url)['instagram']
            try:
                frame['pinterest'] = find_links(contact_link)['pinterest']
            except Exception:
                frame['pinterest'] = find_links(current_url)['pinterest']
            try:
                frame['linkedin'] = find_links(contact_link)['linkedin']
            except Exception:
                frame['linkedin'] = find_links(current_url)['linkedin']
            try:
                frame['youtube'] = find_links(contact_link)['youtube']
            except Exception:
                frame['youtube'] = find_links(current_url)['youtube']
            try:
                frame['twitter'] = find_links(contact_link)['twitter']
            except Exception:
                frame['twitter'] = find_links(current_url)['twitter']
            try:
                try:
                    if len(contact_link) > 0:
                        frame['contact_link'] = contact_link
                except Exception:
                    frame['contact_link'] = find_links(contact_link)['contact_link']
            except Exception:
                frame['contact_link'] = find_links(current_url)['contact_link']
            try:
                frame['faq_link'] = find_links(contact_link)['faq_link']
            except Exception:
                frame['faq_link'] = find_links(current_url)['faq_link']
            try:
                frame['privacy_link'] = find_links(contact_link)['privacy_link']
            except Exception:
                frame['privacy_link'] = find_links(current_url)['privacy_link']
            try:
                frame['delivery_link'] = find_links(contact_link)['delivery_link']
            except Exception:
                frame['delivery_link'] = find_links(current_url)['delivery_link']
            try:
                frame['terms_link'] = find_links(contact_link)['terms_link']
            except Exception:
                frame['terms_link'] = find_links(current_url)['terms_link']
            try:
                frame['return_link'] = find_links(contact_link)['return_link']
            except Exception:
                frame['return_link'] = find_links(current_url)['return_link']
            try:
                frame['warranty_link'] = find_links(contact_link)['warranty_link']
            except Exception:
                frame['warranty_link'] = find_links(current_url)['warranty_link']
            try:
